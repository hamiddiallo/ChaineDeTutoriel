<?php
    ...
    if(isset($_POST['instructeur'])){
        $idInstructeur = implode(',',$_POST['instructeur']);
        supprimerData($pdo,"instructeur",$idInstructeur,"idInstructeur");
    }
    $records_per_page = 10;
    ...
    $total_records_query = $pdo->query("SELECT COUNT(*) FROM instructeur ");
    ...
    $instructeurs = getFromTablesWithOffset($pdo,"instructeur",$records_per_page, $offset);
    if(isset($_POST['filtrer'])){
        if(!empty($_POST['email'])){
            $critere = 'mail=\''.$_POST['email'].'\'';
        }elseif(!empty($_POST['prenom']) && !empty($_POST['nom'])){
            $critere = 'nom=\''.$_POST['nom'].'\',prenom=\''.$_POST['nom'].'\'';
        }elseif(!empty($_POST['nom'])){
            $critere = 'nom=\''.$_POST['nom'].'\'';
        }elseif(!empty($_POST['prenom'])){
            $critere = 'prenom=\''.$_POST['prenom'].'\'';
        }else{
            die('vous n\'avez saisit aucun champ');
        }
        $instructeurs = getFilter($pdo,'instructeur',$records_per_page,$offset,$critere);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="body">
    <nav class="nav-bar">
        <h1><a href="administrateur.php">Dashboard</a><i class="fa-solid fa-bars"></i></h1>
        <ul>
            <li>
            ...
            <li>
                <a href="gestionInstructeur.php" class="activiteEnCours">
    ...
    <main class="content">
        <header class="header-user" style="margin-bottom:25px;">
            <span class="user" style="color:#6c757d">Administrateur</span>
        </header>
        <div class="cheminTutoriel" style="margin-left:20px;">
            <a href="administrateur.php">accueil</a>
            ...
            <a href="gestionInstructeur.php">instructeur</a>
            <div class="ajouterSupprimer">
                <a href="ajouterInstructeur.php" data-tooltip="ajouter" class="tooltip-container"><i class="fa-solid fa-plus"></i></a>
                <button data-tooltip="supprimer" class="tooltip-container"><i class="fa-regular fa-trash-can"></i></button>
            </div>
            <div style="clear:right;"></div>
        </div>
        <div style="margin:25px 10px 20px 21px;display:flex;height:368px">
            <div class="tableau2" style="transform:translateX(0);width:75%;margin-right:30px;">
                <div style="border-bottom:1px solid rgba(0, 0, 0, 0.175);">
                    <i class="fa-solid fa-list"></i>
                    <span>liste d'instructeur</span>
                </div>
                <div style="padding:30px;">
                    <form method="POST" id="suppression">
                        <table>
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <label class="custom-checkbox"> 
                                            <input type="checkbox" name="instructeur[]" value="<?=$instructeur->idInstructeur?>">
                                            <span class="checkmark"></span>
                                        </label>
                                    </td>
                                    <td> <?=$instructeur->nom?> </td>
                                    <td><?=$instructeur->prenom?></td>
                                    <td><?=$instructeur->mail?></td>
                                    <td><a href="modifierUtilisateur.php?instructeur=<?=$instructeur->idInstructeur?>"  data-tooltip="modifier" class="tooltip-container"><i class="fa-solid fa-pencil"></i></a></td>
                                </tr>
                                <?php endforeach?>
                            </tbody>
                        </table>
            ...
            <div class="tableau2" style="width:25%;transform:translateX(0)">
                <div style="border-bottom:1px solid rgba(0, 0, 0, 0.175);">
                    <i class="fa-solid fa-filter"></i>
                    <span>filtrer</span>
                </div>
                <form class="filtrer" method="post">
                        <label for="nom">nom</label>
                        <input type="text" id="nom" name="nom" placeholder="Nom">
                        <label for="prenom">prenom</label>
                        <input type="text" id="prenom" name="prenom" placeholder="Prenom">
                        <label for="email">email</label>
                        <input type="text" id="emai" name="email" placeholder="Email">
                        <button type="submit" class="bouttonSubmit" name="filtrer">
                            <i class="fa-solid fa-filter"></i>
                            filtrer
                        </button>
                </form>
    ...
    <script>
            ...
            firstCheckbox.addEventListener("click",function(){
                ...
                for(let i = 1;i < 11;i++){
                    if(this.checked === true && allCheckBox[i].checked === false){
                        allCheckBox[i].checked = !allCheckBox[i].checked
                    }else if(this.checked === false && allCheckBox[i].checked === true){
                        allCheckBox[i].checked = !allCheckBox[i].checked
                    }    
                }
            })
