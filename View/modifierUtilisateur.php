<?php
    ...
    if (isset($_GET['etudiant'])) {
        $user = getRawById($pdo,"etudiant", $_GET["etudiant"], "idEtud");
        $connected = "etudiant";
        $action="../Controlleur/controlEtudiant.php";
        $chemin = "gestionEtudiant.php";
    }elseif(isset($_GET['instructeur'])){
        $user = getRawById($pdo,"instructeur", $_GET["instructeur"], "idInstructeur");
        $connected = "instructeur";
        $action="../Controlleur/controlInstructeur.php";
        $chemin = "gestionInstructeur.php";
    } else{
...
<body class="body">
    ...
    <main class="content">
        ...
        <div class="cheminTutoriel" style="margin-left:20px;">
            ...
            <a href="<?=$chemin?>"> <?=$connected?></a>
        </div>
        <div class="tableau2" style="transform:translateX(0);margin:25px 0 20px 21px;width:96%;box-sizing:border-box;">
                <div style="text-transform:capitalize;">
                    <i class="fa-solid fa-pencil"></i>
                    <span>modifier <?=$connected?></span>
                </div>
                <div class="formulaire">
                    <div>details  <?=$connected?></div>
                    <form action="<?=$action?>" method="post">
                        <input type="hidden" name="hidden" value="<?= $connected == 'etudiant'?$user->idEtud:$user->idInstructeur?>">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" placeholder="prenom" name="prenom" value="<?=$user->prenom?>">
                        </div> 
                        <div>
                            <label for="nom">mail</label>
                            <input type="email" placeholder="email" name="email" value="<?=$user->mail?>">
                        </div>
                        <button type="submit" class="ajouterEtudiant" name = "modifier">
