<?php
    ...
    if (isset($_SESSION["etudiant"])){
        $etudiant = getRawById($pdo,"etudiant", $_SESSION["etudiant"], "idEtud");
        ...
        $sql = "SELECT * from tutoriel where matiere in (SELECT idMat from etudiant_matiere where idEtud = $etudiant->idEtud) LIMIT :limit OFFSET :offset";
        ...
        $total_records_query = $pdo->query("SELECT COUNT(*) FROM tutoriel where matiere in (SELECT idMat from etudiant_matiere where idEtud = $etudiant->idEtud)");
...
<body <?= isset($_COOKIE['theme'])?'class='.$_COOKIE['theme']:''?>>
    <header class="header-user">
        <span class="user">Etudiant</span>
        <div class="englobe">
            <span id="user-name">
                <span><?=$etudiant->nom?> <?=$etudiant->prenom?></span>
            </span>
            <ul class="fonctionnalite">
                <li><a href="choisirCours.php?ajout"><i class="fa-solid fa-plus"></i>ajouter des matieres</a></li>
    ...
    <main class="container">
        <div class="citation"> 
            ...
            <p style="text-align:justify">sont le languages avec lequel Dieu a écrit l'univers.</p>
        </div>
        <?php foreach($tutos as $tuto):?>
            <section class="tutoriel">
            <a href="tutoriel.php?tuto=<?=$tuto->id_tuto?>&etudiant=<?=$etudiant->idEtud?>">
            <div class="cadre">
                <span class="professeur">Mr Mohamed Diop</span>
