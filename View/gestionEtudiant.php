<?php
    ...
    if(isset($_POST['etudiant'])){
        $idEtud = implode(',',$_POST['etudiant']);
        supprimerData($pdo,"etudiant_matiere",$idEtud,"idEtud");
        supprimerData($pdo,"etudiant",$idEtud,"idEtud");
    }
    $records_per_page = 10;
    ...
    $total_records_query = $pdo->query("SELECT COUNT(*) FROM etudiant ");
    ...
    $etudiants = getFromTablesWithOffset($pdo,"etudiant",$records_per_page, $offset);

    if(isset($_POST['filtrer'])){
        if(!empty($_POST['email'])){
            $critere = 'mail=\''.$_POST['email'].'\'';
        }elseif(!empty($_POST['prenom']) && !empty($_POST['nom'])){
            $critere = 'nom like \''.$_POST['nom'].'\',prenom like \''.$_POST['prenom'].'\'';
        }elseif(!empty($_POST['nom'])){
            $critere = 'nom like \''.$_POST['nom'].'\'';
        }elseif(!empty($_POST['prenom'])){
            $critere = 'prenom like\''.$_POST['prenom'].'\'';
        }else{
            die('vous n\'avez saisit aucun champ');
        }
        $etudiants = getFilter($pdo,'etudiant',$records_per_page,$offset,$critere);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="body">
    <nav class="nav-bar">
        ...
        <ul>
            <li>
                <a href="gestionEtudiant.php" class="activiteEnCours">
    ...
    <main class="content">
        ...
        <div class="cheminTutoriel" style="margin-left:20px;">
            ...
            <a href="gestionEtudiant.php">etudiant</a>
            <div class="ajouterSupprimer">
                <a href="ajouterEtudiant.php" data-tooltip="ajouter" class="tooltip-container"><i class="fa-solid fa-plus"></i></a>
                <button data-tooltip="supprimer" class="tooltip-container"><i class="fa-regular fa-trash-can"></i></button>
            </div>
            <div style="clear:right;"></div>
        </div>
        <div style="margin:25px 10px 20px 21px;display:flex;">
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <label class="custom-checkbox"> 
                                            <input type="checkbox" name="etudiant[]" value="<?= $etudiant->idEtud?>">
                                            <span class="checkmark"></span>
                                        </label>
                                    </td>   
                                    <td> <?=$etudiant->nom?> </td>
                                    ...
                                    <td><?=$etudiant->mail?></td>
                                    <td><a href="modifierUtilisateur.php?etudiant=<?=$etudiant->idEtud?>"  data-tooltip="modifier" class="tooltip-container"><i class="fa-solid fa-pencil"></i></a></td>
                                </tr>
                                <?php endforeach?>
                            </tbody>
                        </table>
            ...
            <div class="tableau2" style="width:25%;transform:translateX(0);height:368px">
                <div style="border-bottom:1px solid rgba(0, 0, 0, 0.175);">
                ...
                <form class="filtrer" method="post">
                        ...
                        <button type="submit" name="filtrer" class="bouttonSubmit">
                            <i class="fa-solid fa-filter"></i>
                            filtrer
                        </button>
                </form>
