...
if(is_array($_SESSION['instructeur'])){
    $matieres = getFromTables($pdo,"matiere");
}else{
    $matieres = getFromTablesWithExcept($pdo, "matiere", "idMat" ,"InstructeurMatiere" , "idInstructeur", $_SESSION['instructeur']);
}
...
<head>
    ...
    <title>Choisir votre spécialité</title>
...
<body <?= isset($_COOKIE['theme'])?'class='.$_COOKIE['theme']:''?>>
    ...
    <main class="container">
        <div class="choix">
            ...
            <form action="../Controlleur/controlInstructeur.php" method="post" enctype="multipart/form-data">
                <?php foreach ($matieres as $matiere): ?>
                    <label>
                        <input type="checkbox" name="expertise[]" value="<?= htmlspecialchars($matiere->idMat) ?>">
                        <span class="checkbox-container"></span>
                        <?= htmlspecialchars($matiere->nomMat) ?>
                    </label>
                <?php endforeach; ?>
                ...
                <div>
                    ...
                    <input id="inputFile" name="certification" type="file" accept="application/pdf" required>
                </div>
