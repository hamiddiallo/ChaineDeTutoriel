<?php
    //revenir pour les affichages avec le javascript
    require '../Fonction/fonction.php';
    require '../Include/bdd.php';
    if(isset($_POST['nom'],$_POST['prenom'],$_POST['email'],$_POST['motDePasse'],$_POST['confirme'])){
        $etudiants = getFromTables($pdo,'instructeur');
        $confirmationMotDePasse = $_POST['motDePasse'] == $_POST['confirme'];
        if($confirmationMotDePasse){
            $status = 1;
            foreach($etudiants as $etudiant){
                if($etudiant->mail == $_POST['email']){
                    $status = 0;
                }
            }
    
            if($status){
                inserer_dans_la_bdd($pdo,'instructeur','nom,prenom,mail,password',[$_POST['nom'],$_POST['prenom'],$_POST['email'],$_POST['motDePasse']]);
                echo 'instructeur ajouté avec succès';
            }else{
                echo 'echec d\'ajout cet email existe deja ';
            }
        }else{
...
<body class="body">
    ...
    <main class="content">
        ...
        <div class="tableau2" style="transform:translateX(0);margin:25px 0 20px 21px;width:96%;box-sizing:border-box;">
                <div style="text-transform:capitalize;">
                    <i class="fa-solid fa-pencil"></i>
                    <span>ajouter instructeur</span>
                </div>
                <div class="formulaire">
                    <div>details instructeur</div>
                    <form  method="post">
                        <div>
                        ...
                        <div style="border:none;font-size:1.5rem;padding:8px">mot de passe</div>
                        <div>
                            <label for="nom">mot de passe</label>
                            <input type="text" placeholder="mot de passe" name="motDePasse" required>
                        </div>
                        <div>
                            <label for="nom">confirmé </label>
                            <input type="text" placeholder="confirmé" name="confirme" required>
                        </div>
                        <button type="submit" class="ajouterEtudiant tooltip-container" data-tooltip="ajouter">
