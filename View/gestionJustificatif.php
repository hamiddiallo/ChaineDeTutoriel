<?php
    ...
    if(isset($_GET['valid'])){
        $params = ["isValid" => 1];
        updateData($pdo, "InstructeurMatiere", $params, $_GET['justif'], "justificatif");
        header('Location: gestionJustificatif.php');
    ...
    $total_records_query = $pdo->query("SELECT COUNT(*) FROM  InstructeursMatiereNonValide");
    ...
    $justifcatif = getFromTablesWithOffset($pdo,"InstructeursMatiereNonValide",$records_per_page, $offset);
...
<body class="body">
    ...
    <main class="content">
        ...
        <div class="cheminTutoriel" style="margin-left:20px;">
            ...
            <a href="gestionEtudiant.php">Gestion des justificatifs</a>
            <div style="clear:right;"></div>
        </div>
        <div style="margin:25px 10px 20px 21px;display:flex;">
            <div class="tableau2" style="transform:translateX(0);width:75%;">
                <div style="border-bottom:1px solid rgba(0, 0, 0, 0.175);">
                    <i class="fa-solid fa-list"></i>
                    <span>liste des justificatifs</span>
                </div>
                ...
                <form method="POST" id="suppression">
                    <table>
                        ...
                        <tbody>
                            <?php foreach($justifcatif as $justif):?>
                            <tr>
                                <td></td>
                                <td> <?=$justif->instructeur?> </td>
                                <td><?=$justif->matieres?></td>
                                <td><a href="gestionExpertise.php?file=<?=$justif->justificatif?>" target="_blank"><butto>afficher</button></a></td>
                                <td><a href="gestionJustificatif.php?valid&justif=<?=$justif->justificatif?>"><i class="fa-solid fa-square-check"></i></a></td>
                                <td><a href="gestionJustificatif.php?cancel&justif=<?=$justif->justificatif?>"><i class="fa-solid fa-xmark"></i></a></td>
                            </tr>
                            <?php endforeach?>
                        </tbody>
                    </table>
